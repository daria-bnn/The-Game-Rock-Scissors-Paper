...
function renderForm(container) {
    ...
    form.addEventListener('submit', (event) => {
        ...
        httpRequest({
            url: `${baseUrl}/login?login=${input.value}`,
            onSuccess: getStatusGame,
        });
    });
...
function renderPlayList(container) {
    httpRequest({
        url: `${baseUrl}/player-list?token=${window.application.token}`,
...
function renderButtonForPlay(container) {
    ...
    button.addEventListener('click', () => {
        ...
        httpRequest({
            url: `${baseUrl}/start?token=${window.application.token}`,
            onSuccess: function (data) {
...
function renderWaitingBlock(container) {
    if (window.application.timers.length === 0) {
        ...
        const timerForStatusGame = setInterval(() => {
            httpRequest({
                url: `${baseUrl}/game-status?token=${window.application.token}&id=${window.application.idGame}`,
                onSuccess: showStatusGame,
            })
        }, 500);
...
function renderBottonsMoves(container) {
    ...
    buttonPaper.classList.add('app__button', 'block-moves__button', 'block-moves__button_papper');
    buttonPaper.dataset.dataMove = 'paper';
    buttonPaper.textContent = 'Бумага';

    const buttonScissors = document.createElement('button');
    buttonScissors.classList.add('app__button', 'block-moves__button', 'block-moves__button_scis');
    buttonScissors.dataset.dataMove = 'scissors';
    buttonScissors.textContent = 'Ножницы';

    const buttonRock = document.createElement('button');
    buttonRock.classList.add('app__button', 'block-moves__button', 'block-moves__button_rock');
    ...
    blockMoves.addEventListener('click', function (event) {
        ...
        httpRequest({
            url: `${baseUrl}/play?token=${window.application.token}&id=${window.application.idGame}&move=${target.dataset.dataMove}`,
            onSuccess: showResultGameForButton,
        })
    })
